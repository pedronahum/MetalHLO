...
- **`jnp.unique`**: not supported. Its multi-stage lowering (`lexsort` + `cumsum` + masking + scatter) is individually correct on every op, but the composed result is wrong because the static memory planner reuses a buffer still live across the pipeline (`METALHLO_NO_REUSE=1` produces the correct result). The fix is in the planner's liveness analysis. (`sort`, `argsort`, and `lexsort` themselves *are* supported and verified.) `triangular_solve` with `left_side=false` (`x·A=b`) is also not yet implemented.
