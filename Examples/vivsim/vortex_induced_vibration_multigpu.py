"""
Vortex-Induced Vibration (VIV) of a circular cylinder using IB-LBM with Multi-GPU — MetalHLO backend test
...
NOTE: MetalHLO currently supports single-device execution only.
      This example will run on a single device if multi-device sharding
      is not available. It is included to test MetalHLO compatibility with
      the shard_map API.

Note: `NY` must be evenly divisible by `N_DEVICES`.

Adapted from: https://github.com/haimingz/vivsim/tree/main/examples
"""

# --- MetalHLO backend configuration ---
import os
os.environ.setdefault("JAX_PLATFORMS", "metalhlo,metal,cpu")
...
if PLOT:
    ...
    plt.suptitle("VIV Multi-GPU (MetalHLO backend)")
    plt.tight_layout()
