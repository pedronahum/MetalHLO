"""
Karpathy's "atomic GPT" — pure dependency-free Python, with hand-written
autograd, trained on names.

This file mirrors @karpathy's gist verbatim:
    https://gist.github.com/karpathy/8627fe009c40f57531cb18360106ce95

The only additions are an argparse interface (--steps, --skip-inference)
and wall-clock timing around the training loop so it slots into our
MetalHLO comparison benchmark.
"""
...
parser.add_argument(
    "--skip-inference",
    action="store_true",
    help="Skip the 20-sample inference loop at the end (training-only timing).",
)
parser.add_argument(
    "--print-every",
    type=int,
...
print()  # newline after the \r-updating progress line
...
print(
    ...
    f"(mean step after {warmup}-step warmup: {mean_steady*1000:.1f}ms)"
)
